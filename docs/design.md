...
| `inaturalist_search_observations` | Search georeferenced wildlife sightings by area, date, taxon, quality grade, annotation, conservation status, observer, project, and licence. Returns a projected record per sighting with coordinates, licence, photo, and identification counts. | `place_id` \| `lat`+`lng`+`radius` \| bbox, `taxon_id`, `d1`/`d2`, `quality_grade`, `captive`, `term_id`/`term_value_id`, `iconic_taxa`, `user_id` \| `user_login`, `project_id`, `license`/`photo_license`, `page` \| `cursor`, `per_page`, `include` | `readOnlyHint: true`, `openWorldHint: true` |
...
| `inaturalist_get_species_counts` | Rank the distinct species recorded in an area and period, most-observed first. Answers "what lives here" without paging through individual sightings. | `place_id` \| `lat`+`lng`+`radius` \| bbox, `d1`/`d2`, `taxon_id`, `quality_grade`, `user_id` \| `user_login`, `project_id`, `per_page` | `readOnlyHint: true`, `openWorldHint: true` |
| `inaturalist_get_histogram` | Build a phenology histogram for a taxon in an area — which months, weeks, or years it is recorded in. | `taxon_id`, `place_id` \| `lat`+`lng`+`radius` \| bbox, `interval`, `date_field`, `term_id`/`term_value_id`, `iconic_taxa` | `readOnlyHint: true`, `openWorldHint: true` |
...
- Search observations by point-and-radius, place id, or bounding box; by date range, taxon, quality grade, annotation, conservation status, rank band, iconic taxon, observer, project, licence, and captivity.
...
Sizes are the whole `tools/call` reply. Nothing is lost at the lower caps: `page` reaches every record in the first 10,000 under any ordering, an id-descending walk continues past that by `cursor` (see Pagination), and two leaderboard pages of 250 cover the whole 500-entry window those endpoints rank. `inaturalist_find_places` (max 30; ten places measured 6,752 bytes, so ~675 each) and `inaturalist_get_similar_species` (max 50, over look-alikes projected to roughly 250 bytes each) already sit inside both thresholds and are unchanged. `include: ["photos"]` on an observation search multiplies the per-record cost and is the caller's own call to make — the field's description says as much. `inaturalist_get_histogram` has no page/default split — it is a single response, so the cap targets the 50,000-byte advertised maximum directly rather than the smaller default; the first 800 buckets in upstream key order are kept, `total` still sums every bucket upstream returned, and the `truncated`/`shown`/`cap` enrichment discloses the cut the way every other list tool does.
...
| `page × per_page > 10,000` | **403** `{"error":"Result window is too large, page x size must be less than or equal to [10000]. Please narrow your search, or use a sliding window approach with id_above or id_below params.","status":403}` | Rejected in-process before the request, as a typed error whose recovery names the one way past it: re-run ordered by `id` descending and follow `next_cursor`, or narrow the filters. |
...
| `user_id=999999999`, `user_login=zz-no-such-login-xq9` — verified 2026-09-24 | **422** `{"error":"Unknown user_id 999999999","status":422}` — an unknown login is worded the same way, `Unknown user_id zz-no-such-login-xq9`, on `/observations` and `/observations/species_counts` alike | Mapped to a typed `unknown_user` routing to `inaturalist_resolve_name` type `user`. A whitespace-only login answers the same 422, so it needs no rule of its own. |
| `project_id=999999999` — verified 2026-09-24 | **422** `{"error":"Unknown project_id: [999999999]","status":422}` | Mapped to a typed `unknown_project_id` routing to `inaturalist_resolve_name` type `project`. `project_id` is a positive integer in the schema; upstream also accepts a slug, but `inaturalist_resolve_name` returns the id. |
| `user_login=` or `project_id=` (blank) — verified 2026-09-24 | 200, `total_results: 388,573,266` — the whole global index | A blank `user_login` reads as unset, like a blank `q` or `d1`; `project_id` is an integer and cannot be blank. |
| `user_id=1&user_login=loarie` (a mismatched pair) — verified 2026-09-24 | 200, `total_results: 0` | Rejected in-process as `conflicting_observer`: a matching pair is redundant and a mismatched one matches nothing. |
| `license=bogus`, `license=null`, `license=cc_by` — verified 2026-09-24 | 200, `total_results: 0`. `license=CC0` and `CC-BY` answer exactly as `cc0` and `cc-by` | `license` and `photo_license` are arrays over the seven `LICENSE_CODES`, each value lowercased before the enum; anything else — the `licenses` topic's `null` entry included — is refused at the schema. |
| `term_value_id` without `term_id` on `/observations/histogram` — verified 2026-09-24 | 200, the unfiltered curve (Jan / Jul 3,254 / 8,399 for taxon 48662 in place 14, with or without it) | Rejected in-process as `unpaired_annotation_value`, the same rule as search and species counts. A `term_value_id` from another attribute (`term_id=1&term_value_id=13`) zeroes every bucket, which the zero-hit notice names. |
...
- **`page`** (1-based, default 1) walks the first 10,000 results under any ordering. `page × per_page > 10,000` is refused in-process rather than sent.
...
- **`next_cursor` is issued only when the applied ordering is `id desc`** — requested outright, or forced by a cursor. Under any other ordering `id_below` is not a continuation: verified 2026-09-24 on `place_id=1&taxon_id=48662&per_page=3`, an `observed_on asc` page (1899-11-30, 1899-11-30, 1958-07-04) continued by its last id jumped to 2021, a default-ordered page's cursor repeated one of its own records, and an `id asc` page's cursor (6171) returned page 1 again, reversed. `id asc` is excluded too, since only `id_below` is sent. Every other ordering gets `Raise page to {n} to continue`, and a page whose successor would cross 10,000 gets `re-run with order_by "id" and order "desc" and follow next_cursor, or narrow the filters` — the same route the `result_window_exceeded` recovery names.
- **An empty page with `total_results > 0` is past the end, not a zero-hit.** On the page path of `inaturalist_search_observations`, and on `inaturalist_get_species_counts` and `inaturalist_get_leaderboard`, the notice names the last page holding results, `ceil(total_results / per_page)`, and says the filters did not empty it. A page at or before that one that still comes back empty means upstream's count ran ahead of its rows, and the notice says so rather than naming a "last page" the request did not pass. The zero-match guidance applies only when `total_results` is 0.
...
With `taxon_id`, the `controlled_terms` topic adds an `observed_usage` array from `GET /observations/popular_field_values?taxon_id=`, projected to `{ attribute, term_id, value, term_value_id, count }` from `controlled_attribute.{label,id}`, `controlled_value.{label,id}`, and `count` — for taxon 48662 that is Life Stage (1) = Adult (2) 336,576, Alive or Dead = Alive 128,365, Life Stage = Larva 127,161, and so on. The ids are named for the filter parameters they feed, and they ride because labels repeat across attributes: "Egg" is value 7 under Life Stage and value 30 under Evidence of Presence, and "Cannot Be Determined" is value 20 under both Sex and Alive or Dead. A missing id is `null`, never invented. The per-entry `month_of_year` breakdown is dropped.
...
**format():** the topic name and its source as a header line, then one `- ` bullet per entry — `**{label}** (id {id}) — {value label} ({value id}), …` for `controlled_terms`, `**{code}** — {label}. {notes}` for the static topics. When `observed_usage` is present it follows as its own `### Observed usage` list, `{attribute} = {value} — {count} observations · term_id {term_id} · term_value_id {term_value_id}`, most-used first, with `not published` for a null id.
...
| `user_id` | int ≥ 1, optional | `user_id` | One observer, from `inaturalist_resolve_name` type `user` → `id`. Mutually exclusive with `user_login`. |
| `user_login` | string, optional | `user_login` | One observer, by the `login` on a resolve_name user candidate, a leaderboard entry, or an observation's `observer`. A blank value reads as unset. Mutually exclusive with `user_id`. |
| `project_id` | int ≥ 1, optional | `project_id` | One project, from `inaturalist_resolve_name` type `project` → `id`. |
| `licensed` | boolean, optional | `licensed` | The observation's own licence is not null — any licence, the NC and ND variants included. |
| `photo_licensed` | boolean, optional | `photo_licensed` | At least one photo's licence is not null. |
| `license` | enum array, optional | `license` | The observation's own `license_code` is one of these — the seven `LICENSE_CODES` (`cc-by`, `cc-by-nc`, `cc-by-nd`, `cc-by-sa`, `cc-by-nc-nd`, `cc-by-nc-sa`, `cc0`), comma-joined as OR. Each value is lowercased before the enum, since upstream reads `CC0` as `cc0`. Probed on `place_id=14&quality_grade=research`: `cc0` 668,197, `cc-by` 1,483,169, `cc-by,cc0` 2,151,366 of 16,274,317. |
| `photo_license` | enum array, optional | `photo_license` | At least one photo carries one of these codes, matched independently of the record's own licence — a `license=cc-by&photo_license=cc0` record came back with `license_code: "cc-by"` and `cc0` photos. |
...
| `order_by` | enum, default `observed_on` | `order_by` | `created_at` \| `geo_score` \| `id` \| `observed_on` \| `random` \| `species_guess` \| `updated_at` \| `votes`. Forced to `id` when `cursor` is supplied. `id` with `order: "desc"` is the only ordering that issues `next_cursor` — see Pagination. |
| `order` | enum, default `desc` | `order` | |
| `page` | int ≥ 1, default 1 | `page` | Refused in-process when `page × per_page > 10000`. |
| `cursor` | string, `^[1-9]\d*$`, optional | `id_below` | The `next_cursor` from a previous id-descending page — always a decimal observation id. A non-numeric value 500s upstream, so the schema enforces the pattern, the same way `place_id` enforces an integer. Mutually exclusive with `page`. |
...
**Output:** `total_results`, `observations[]` (the projected record), `next_cursor` (string, optional — only when `has_more` and the applied ordering is `id desc`), `has_more` (boolean).
...
| `result_window_exceeded` | `ValidationError` | `page × per_page` would exceed the upstream 10,000-result window. | `Past 10,000 results only the cursor continues, and only on an id-descending walk: re-run with order_by "id" and order "desc", then pass each page's next_cursor as cursor. Otherwise narrow the filters to bring the records into the first 10,000.` |
...
| `conflicting_observer` | `ValidationError` | `user_id` and `user_login` were both supplied. | `Pass one observer, as user_id or user_login — each names the observer on its own, and a mismatched pair matches nothing.` |
...
| `unknown_user` | `ValidationError` | Upstream answered 422 `Unknown user_id`, which it also says for an unknown `user_login`. | `Resolve the observer with inaturalist_resolve_name type user and pass the id or login it returns.` (`thrownBy: 'service'`) |
| `unknown_project_id` | `ValidationError` | Upstream answered 422 `Unknown project_id`. | `Resolve the project name with inaturalist_resolve_name type project and pass the id it returns.` (`thrownBy: 'service'`) |
...
| An observer was given | `No sightings by that observer match the other filters. Confirm {user_id \| user_login} with inaturalist_resolve_name type user, or drop it.` |
| A project was given | `No sightings in that project match the other filters. Confirm project_id with inaturalist_resolve_name type project, or drop it.` |
...
No fragment names a filter the call did not supply. These apply only when `total_results` is 0; an empty page with matches upstream gets the past-the-end notice instead (see Pagination) — `Page 40 is past the end: 93 records match, so the last page holding results at per_page 25 is 4. Request page 4 or lower — the filters are not what emptied this page.`

**Truncation:** `truncated: false` with `shown` and `cap` on every other path; when the page fills `per_page` and more remain, `ctx.enrich.truncated({ shown, cap, guidance })`. Under `id desc` the guidance names `next_cursor`; under any other ordering it names the next page, or — when that page would cross 10,000 — the re-run ordered by `id` descending.
...
| `user_id` / `user_login` / `project_id` | as on `inaturalist_search_observations` | same | One observer or one project; the same pairing rule and blank-login handling. Probed with no quality or captive filter: `user_id=1` and `user_login=kueda` both 11,698 species, `project_id=227779` 73,833, against 597,770 unfiltered. |
...
**Output:** `total_results` (distinct species matching), `species[]` — `{ position, taxon_id, name, common_name, rank, iconic_taxon_name, observation_count, photo }`, from `results[].count` and `results[].taxon`. `position` is the row's absolute place in the ranking, counted from page 1: the service computes `(page − 1) × per_page + index + 1` from the raw row index, as `getLeaderboard` does, so a row dropped for carrying no taxon leaves a gap rather than renumbering the rest. `rank` stays the taxonomic rank.

**format():** `total_results` as a header line, then a numbered list ranked by `observation_count` — `{position}. **{common_name}** (*{name}*) — {observation_count} observations · {rank} · {iconic_taxon_name} · taxon_id {taxon_id}` — with the photo block indented under each entry. Page 3 at `per_page` 3 numbers its rows 7, 8, 9.

**Errors:** `invalid_geography`, `inverted_date_range`, `unpaired_annotation_value`, `conflicting_observer`, `unknown_taxon_id`, `unknown_user`, `unknown_project_id` — same reasons, codes, and recovery strings as on `inaturalist_search_observations`.
...
**Zero-hit notice:** `No species recorded for those filters.`, followed by one remedy per narrowing filter the call actually supplied, in this order: `widen or drop d1/d2`, `widen the area`, `drop taxon_id or confirm it with inaturalist_resolve_name`, `drop {user_id | user_login} or confirm the observer with inaturalist_resolve_name`, `drop project_id or confirm it with inaturalist_resolve_name`, and `set quality_grade to include "needs_id"` unless it already does. It applies only when `total_results` is 0; an empty page past the end names the last page instead (see Pagination) — `page: 2000, per_page: 50` on `place_id: 1` reads `Page 2000 is past the end: 99891 species match, so the last page holding results at per_page 50 is 1998.` With `place_id` and `taxon_id` under the default quality grade: `No species recorded for those filters. Widen the area, drop taxon_id or confirm it with inaturalist_resolve_name, or set quality_grade to include "needs_id".` `inaturalist_get_leaderboard` composes its notice the same way.
...
| `quality_grade` | enum array, default `["research"]` | `quality_grade` | |
| `captive` | boolean, default `false` | `captive` | |
| `term_id` / `term_value_id` | int arrays, optional | same | One life stage or reproductive state — the curve per annotation pair. Same pairing rule as search. For taxon 48662 in place 14, research grade and wild: unfiltered Jan / Jul 3,161 / 8,190, Larva (`1`/`6`) 721 / 1,985, Adult (`1`/`2`) 2,129 / 5,062. |
| `iconic_taxa` | enum array, optional | `iconic_taxa` | The 14 spec values, comma-joined as OR. Place 14 unfiltered sums 16,274,309; `Aves` 3,912,205, `Insecta` 2,106,479. |
...
**Errors:** `invalid_geography`, `inverted_date_range`, `unpaired_annotation_value`, `unknown_taxon_id` — same strings.
...
**Zero-hit notice** (every bucket zero, computed over the full unclipped set), composed from the call rather than fixed: with `taxon_id` set and no date range it reads `Every bucket is zero — this taxon has no records in that area. Confirm the taxon with inaturalist_resolve_name, or widen the area.` The taxon wording is dropped when `taxon_id` was omitted (`nothing is recorded in that area. Widen the area, or relax quality_grade or captive.`); when `d1` or `d2` was set, the range is named as the likeliest cause (`no records of this taxon in that area fall in {d1}…{d2}. Widen or drop d1/d2.`, with `any start`/`any end` for an open bound); otherwise an annotation or iconic filter is (`no records of this taxon in that area match the annotation filter.`, or `match iconic_taxa Aves, Insecta.`); and the area clauses drop when no area was given. An annotation filter always adds `Check that each term_value_id belongs to its term_id with inaturalist_list_reference topic controlled_terms{ and taxon_id}, or drop the annotation filter.` — a value from another attribute (`term_id=1&term_value_id=13`) zeroes every bucket — and `iconic_taxa` adds `Drop iconic_taxa or choose another group.` It replaces the truncation guidance when the answer is genuinely zero everywhere, since narrowing the range would not help — the notice is written once, as the truncation guidance itself on a capped response.
...
**Zero-hit notice:** `Nobody has recorded observations matching those filters.` (`Nobody has made identifications…` for `kind: "identifiers"`), followed by the remedies for the filters the call supplied, composed as on `inaturalist_get_species_counts` — `taxon_id`, the date range, and the area are named only when given. It applies only when `total_results` is 0; an empty page with people matching gets the past-the-end notice (see Pagination). Inside the 500-entry window a page is never past the end of a field of 500 or more, since `leaderboard_window_exceeded` refuses any page past it first.
...
| **422 mapping** | `mapUpstreamError()` matches the body. `{"error":"Unknown taxon_id N","status":422}` is mapped to `data: { reason: 'unknown_taxon_id' }`, `{"error":"Taxon N is not genus or finer","status":422}` from `/identifications/similar_species` to `data: { reason: 'taxon_rank_too_coarse' }`, `Unknown user_id X` (also upstream's wording for an unknown `user_login`) to `unknown_user`, and `Unknown project_id: [X]` to `unknown_project_id`, all `ValidationError`, not retryable, with the recovery from the calling tool's contract and no upstream path in `data` — so the tool contract's reason reaches the wire. `/taxa/abc`'s bare `{"error":"Error","status":422}` never occurs, because the schema rejects the non-integer first. |
...
| **Both `page` and `cursor` are exposed, and supplying both is an error.** | `page` is what an agent reaches for and is correct inside the 10,000-record window under any ordering; `cursor` is the only thing that works past it, and only on an id-descending walk. A silent precedence rule between them would make a deep page quietly return the wrong records. |
...
| **`next_cursor` is issued only when the applied ordering is `id desc`; every other ordering is pointed at the next page.** | A cursor from any other page repeats or skips records — a default-ordered page's cursor returned one of its own records again, and an `id asc` page's cursor returned page 1 reversed and ended the walk. Emitting a token that is not a continuation is worse than emitting none. `id asc` stays excluded rather than gaining an `id_above` cursor, which would be a new parameter, not this fix. `created_at desc` tracked `id desc` on every sample row, but upstream does not document the equivalence, so it gets no cursor either. |
| **An empty page with `total_results > 0` gets a past-the-end notice naming the last page, not the zero-match guidance.** | `species_counts?place_id=1&per_page=50&page=2000` answered 200 with no rows beside `total_results: 99,891`, and the old notice told the agent to widen filters that were not the cause. The last page is `ceil(total_results / per_page)`. A page at or before it that still came back empty is told apart rather than called "past the end", since naming a last page it has not passed would contradict itself. The leaderboard's 500-entry window needs no term in the formula: `leaderboard_window_exceeded` refuses any page past 500 before it is sent, so a field of 500 or more never produces a past-the-end page. |
| **Species-count rows carry an absolute `position`, not a second `rank`.** | `rank` already holds the taxonomic rank on these rows, so reusing the leaderboard's field name would break the output contract, and renaming `rank` would break it the other way. The service computes the position from the page offset and the raw row index, as the leaderboard does, so a dropped taxonless row leaves a gap instead of renumbering every row after it. |
| **`user_id` and `user_login` together are refused as `conflicting_observer`.** | A matching pair is redundant, and a mismatched pair answers zero with HTTP 200 — the silent-narrowing shape this server rejects everywhere else. Either field alone covers every use. `project_id` stays an integer: upstream also takes a slug, but `inaturalist_resolve_name` returns the id and accepting slugs later is additive. |
| **Licence-code filters take the seven `LICENSE_CODES`, lowercased before the enum.** | Upstream answers `bogus`, `null`, and `cc_by` with zero and no error, so the enum is strict; it reads `CC0` and `CC-BY` exactly as `cc0` and `cc-by`, so a case variant is a certain one-to-one mapping and is normalised rather than rejected. The `licenses` topic's `null` entry is not a filter value, and a test pins `LICENSE_CODES` to that topic minus it. |
| **The histogram takes the annotation pair and `iconic_taxa`, and `observed_usage` rows carry `term_id` / `term_value_id`.** | Phenology is usually asked per life stage or flowering state, and the allowlist already carried the three filters. Labels alone could not feed them back: "Egg" and "Cannot Be Determined" each name values under two attributes. Checking that a value belongs to its attribute in-process would cost a `/controlled_terms` lookup per call, so the zero-hit notice names the pairing check instead. |
